...
function go(id){$$(".page").forEach(x=>x.classList.toggle("active",x.id===id));$$(".nav").forEach(x=>x.classList.toggle("active",x.dataset.page===id));$("#pageTitle").textContent={home:"Command Center",sentinel:"Trafficking Sentinel",network:"Criminal Network",cases:"Cases & Leads",ledger:"Evidence Ledger",sources:"Authentic Sources"}[id];load(id)}
...
async function loadOverview(){
 ...
 $("#metrics").innerHTML=[
  ["India HT cases / 2023",d.metrics.indiaCases2023,"NCRB / OGD"],
  ["Victims reported / 2023",d.metrics.victims2023.toLocaleString(),"NCRB / OGD"],
  ["AHT units / 2020",d.metrics.ahtu2020,"MHA"],
  ["Demo graph entities",d.metrics.networkEntities,"Synthetic records"]
...
function selectEntity(e){$("#avatar").textContent=e.type[0].toUpperCase();$("#entityName").textContent=e.name;$("#entityType").textContent=e.type.toUpperCase()+" / DEMO";$("#risk").textContent=e.risk+"%";$("#riskbar").style.width=e.risk+"%";$("#links").textContent=e.links}
...
$("#trace").onclick=()=>toast("Trace generated from synthetic graph");
$("#addEntity").onclick=async()=>{const name=prompt("Synthetic entity name");if(!name)return;const e=await api("/api/entities",{method:"POST",body:JSON.stringify({name,type:"event",risk:35})});state.network=null;await loadNetwork();toast("Synthetic entity added")};
$("#newCase").onclick=async()=>{await api("/api/cases",{method:"POST",body:JSON.stringify({priority:"HIGH"})});state.cases=null;await loadCases();toast("Demo case created")};
load("home");
